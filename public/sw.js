const CACHE_NAME='zodibuddy-v48';
const CDN_ASSETS=[
...
self.addEventListener('fetch',e=>{
  ...
  // CDN assets (React, fonts): cache-first (they never change)
  if(isCDN){
  ...
  // Own app files (app.js, etc): network-first so deployments update immediately
  e.respondWith(fetch(e.request).then(r=>{if(r.ok){const cl=r.clone();caches.open(CACHE_NAME).then(ca=>ca.put(e.request,cl));}return r;}).catch(()=>caches.match(e.request)));
});
